...
func start() *state {
	...
	go func() {
		...
		for {
			...
			pc, err := view.Render(ctx, s.scenes[s.currentScene].W, view.NewCameraAt(width, width, fov, s.loc.At, s.loc.LookingAt), 5, int(float64(runtime.NumCPU())/4), coordinate_supplier.Random)
			if err != nil {
