...
func (c *Cone) Intersect(r geom.Ray, i *Intersections) {
	Intersect(r, i, c.t, c.LocalIntersect)
}

func (c *Cone) LocalIntersect(r geom.Ray, i *Intersections) {
	...
	if geom.AlmostEqual(0, a) {
		...
		if b != 0 {
			...
			i.Add(NewIntersection(-cc/(2*b), c))
		}
	} else {
		// check sides
		disc := b*b - 4*a*cc
...
func (c *Cone) intersectCaps(r geom.Ray, xs *Intersections) {
